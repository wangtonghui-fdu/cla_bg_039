...
match_set(const char *re, int re_len, const char *s, struct regex_info *info)
{
	...
	while (len <= re_len && re[len] != ']' && result <= 0)
	{
		...
		if (re[len] != '-' && re[len + 1] == '-' && re[len + 2] != ']' &&
			re[len + 2] != '\0')
		{
			result = info->flags && IGNORE_CASE ? *s >= re[len] && *s <= re[len + 2] : tolower((int)*s) >= tolower((int)re[len]) && tolower((int)*s) <= tolower((int)re[len + 2]);
			len += 3;
		}
		else
		{
...
doh(const char *s, int s_len, struct regex_info *info, int bi)
{
	...
	do
	{
		...
		len = b->num_branches == 0 ? b->len : i == b->num_branches ? b->ptr + b->len - p
	...
	for (i = 0; i < re_len; i += step)
	{
		...
		else if (re[i] == '\\')
		{
			...
			if (re[i + 1] == 'x')
			{
				...
				FAIL_IF(re[i + 1] == 'x' && !(isxdigit((unsigned char)re[i + 2]) && isxdigit((unsigned char)re[i + 3])),
						SLRE_INVALID_METACHARACTER);
			}
			else
			{
...
int main()
{
	...
	for (j = 0; j < RPT; j++)
	{
		...
		for (i = 0; i < 4; ++i)
		{
			res += slre_match(regexes[i], text, len, &captures, 1);
		}
	}
