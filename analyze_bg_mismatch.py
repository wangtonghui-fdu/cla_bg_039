...
def parse_task8_address(run_dir: Path) -> int | None:
    for release in ("Release_w", "Release_wo"):
        map_path = run_dir / "work" / release / "F280039_driver_core0.map"
...
def parse_disassembly(run_dir: Path) -> dict[int, str]:
    for release in ("Release_w", "Release_wo"):
        dis_path = run_dir / "work" / release / "F280039_driver_core0.S.dis"
        if not dis_path.exists():
            dis_path = run_dir / "work" / release / "F280039_driver_core0.dis"
        if not dis_path.exists():
            continue
        dis: dict[int, str] = {}
