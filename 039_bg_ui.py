#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Minimal PySide6 launcher for the 039 CLA BG local flow."""
...
def dispatch_cli_subcommand() -> int | None:
    ...
    command = sys.argv[1]
    try:
        if command == "--run-039-bg":
            import run_039_bg

            return run_039_bg.main(sys.argv[2:])
...
def friendly_process_error(text: str) -> str:
    ...
    if "timed out" in compact.lower():
        return "流程超时，请检查本地构建、WSL、SSH 或远端仿真是否卡住，或调大仿真时间。"
...
class RunWorker(QObject):
    ...
    def _run_once(self, index: int, timeout_s: int) -> dict[str, Any]:
        ...
        if getattr(sys, "frozen", False):
            cmd = [sys.executable, "--run-039-bg", "--instr", str(self.instr_count), "--sim-timeout", str(timeout_s)]
        else:
...
class LocalEnvWorker(QObject):
    ...
    def _check_local_files(self) -> None:
        required_files = [
            SCRIPT_DIR / "config_039.json",
            SCRIPT_DIR / "random_test" / "instr_generate" / "main.py",
            SCRIPT_DIR / "random_test" / "toolchains" / "bin" / "as.exe",
            SCRIPT_DIR / "random_test" / "toolchains" / "bin" / "ld.exe",
            SCRIPT_DIR / "random_test" / "toolchains" / "bin" / "objcopy.exe",
            SCRIPT_DIR / "random_test" / "toolchains" / "bin" / "objdump.exe",
            SCRIPT_DIR / "random_test" / "toolchains" / "bin" / "gen_ram_image.py",
        ...
        required_dirs = [
            SCRIPT_DIR / "cache" / "039_bg_template" / "Release_w",
            SCRIPT_DIR / "cache" / "039_bg_template" / "Release_wo",
            SCRIPT_DIR / "cache" / "039_bg_template" / "ldscript",
            SCRIPT_DIR / "cache" / "039_bg_template" / "libv2",
        ...
        self.log.emit("本地脚本、随机生成器、039 Windows 工具链、reference_sim 和模板 cache 已满足。\n")
    ...
    def _check_wsl(self) -> None:
        ...
        command = (
            ...
            "echo WSL_PWD=$(pwd); "
            "echo WSL_TOOL_LIST; "
            "ls -l reference_sim/toolchains/bin reference_sim/toolchains/tools reference_sim/simulator 2>&1 || true; "
            "missing=0; "
        ...
        if result.returncode != 0 or "WSL_OK" not in result_output:
            ...
            raise RuntimeError(
                "WSL 可启动，但无法访问当前程序目录或缺少 reference_sim 工具。\n"
                f"WSL 路径: {bash_path}\n"
                "请确认该目录位于 Windows 本地磁盘，并且 reference_sim 目录完整。"
            )
        self.log.emit("WSL 可用，且能访问当前程序目录和 reference_sim 工具。\n")
