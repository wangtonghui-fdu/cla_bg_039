...
DEFAULT_CONFIG = SCRIPT_DIR / "config_039.json"
...
def validate_template(cache_dir: Path, required_dirs: list[str], require_marker: bool = True) -> None:
    ...
    if require_marker and cache_dir.exists() and not marker.exists():
        raise RuntimeError(
            ...
            f"Run: python {SCRIPT_DIR / 'run_039_bg.py'} --bootstrap"
        )
    ...
    if missing:
        raise RuntimeError(
            ...
            + ", ".join(missing)
            + f"\nRun: python {SCRIPT_DIR / 'run_039_bg.py'} --bootstrap"
        )
...
def copy_template_to_work(cache_dir: Path, run_dir: Path) -> Path:
    ...
    # The 039 linker script uses plain INCLUDE directives, and this linker
...
def link_release(cfg: dict[str, Any], work_dir: Path, release: str) -> None:
    if local_build_backend(cfg) == "wsl":
        ...
        project_out = cfg["local"].get("project_out", "F280039_driver_core0.out")
        project_map = cfg["local"].get("project_map", "F280039_driver_core0.map")
...
def upload_release_images(client, cfg: dict[str, Any], remote: RemoteConfig, release_dir: Path) -> None:
    sftp = client.open_sftp()
    try:
        if cfg.get("bootloader_uploads_enabled", False):
...
def default_bootloader_candidates(name: str) -> list[Path]:
    return [
        SCRIPT_DIR / "IDE" / "software_lib_driver_2803x-main" / "bootloader" / "Release" / name,
        SCRIPT_DIR / "software_lib_driver_2803x-main" / "bootloader" / "Release" / name,
        REPO_ROOT / "cla_bg_039_local" / "software_lib_driver_2803x-main" / "bootloader" / "Release" / name,
    ]
...
def copy_key_artifacts(cfg: dict[str, Any], work_dir: Path, run_dir: Path) -> None:
    patterns = [str(item) for item in cfg["local"].get("artifact_patterns", [
        "task8.o",
        cfg["local"].get("project_out", "F280039_driver_core0.out"),
        cfg["local"].get("project_map", "F280039_driver_core0.map"),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run 039 CLA BG local build flow; remote only runs simv.")
